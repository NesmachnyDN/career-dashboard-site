...
const RU = {
  ...
  // Record-specific legacy translations are loaded into the private encrypted snapshot.
...
const ru = (value) => {
  ...
  return snapshot?.display_translations?.[text] || RU[text] || text;
};
...
function startApp() {
  ...
  const isDemo = snapshot.demo?.is_demo === true;
  if (isDemo) {
    currentView = 'demo';
    document.body.classList.add('demo-mode');
    const demoButton = document.createElement('button');
    demoButton.dataset.view = 'demo';
    demoButton.textContent = 'О демо';
    $('#nav').prepend(demoButton);
    const pipelineLaunch = $('#pipeline-launch');
    if (pipelineLaunch) pipelineLaunch.hidden = true;
  }
  $('#generated-at').textContent = isDemo
    ? `Synthetic demo · ${fmtDate(snapshot.generated_at)}`
    : `Снимок: ${fmtDate(snapshot.generated_at)}`;
...
function renderDemo() {
  ...
  const architecture = (demo.architecture || []).map(item => `<li>${esc(item)}</li>`).join('');
  const governance = (demo.governance || []).map(item => `<li>${esc(item)}</li>`).join('');
  const tradeoffs = (demo.tradeoffs || []).map(item => `<li>${esc(item)}</li>`).join('');
  return `
    <div class="callout"><strong>${esc(demo.label || 'Публичная демо-версия')}</strong><br>${esc(demo.message || '')}</div>
    <div class="grid metrics-grid">
      ${metric('Synthetic vacancies', snapshot.analytics?.vacancy_count || 0)}
      ${metric('Synthetic opportunities', snapshot.analytics?.opportunity_count || 0)}
      ${metric('Workflow runs', snapshot.automation?.runs?.length || 0)}
      ${metric('External actions', demo.safety?.external_actions_enabled ? 'enabled' : 'disabled')}
    </div>
    <div class="grid two-col">
      <div class="card"><h2>Архитектура</h2><ul>${architecture}</ul></div>
      <div class="card"><h2>Governance / human control</h2><ul>${governance}</ul></div>
      <div class="card"><h2>Ключевые trade-offs</h2><ul>${tradeoffs}</ul></div>
      <div class="card"><h2>Граница публичной версии</h2><p>В этой версии используются только synthetic records. Внешние действия отключены, а публикация проходит автоматическую проверку на утечки.</p><p><a href="about.html" target="_blank" rel="noopener">Открыть architecture case study →</a></p></div>
    </div>
  `;
}
